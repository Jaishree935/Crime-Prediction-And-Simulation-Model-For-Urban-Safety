...
class WeatherModel():
    ...
    # dataset last available date
        last_date = self.x.index[-1]

        try:
            # try getting weather for selected date
            weatherFeatures = self.x.loc[date].values
        except:
        # if date not found (future date)
            weatherFeatures = self.x.loc[last_date].values
    ...
    def getWeatherFactor(self, date):
        ...
        try:
            ...
            crimeCountPred = self.predict(self.x.index[-1])
